...
create table if not exists dev_ensamble (
  id   text primary key,
  data jsonb
);
...
alter table dev_ensamble enable row level security;
...
create policy "dev_ensamble_auth" on dev_ensamble for all to authenticated using (true) with check (true);
